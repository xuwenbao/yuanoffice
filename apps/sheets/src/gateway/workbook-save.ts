...
import {
  ...
  toSaveVisualAdds,
  toSaveVisualEdits,
} from '../renderer/edit-journal'
import { activeCsvSheet, handleExportCsv, serializeActiveSheetCsv } from '../renderer/csv-export'
...
import {
  ...
  collectFilterStates,
  collectNoteStates,
} from '../renderer/univer-sync'
import type { LazyWorkbookState, UniverRuntime } from '../renderer/univer-state'
import { sheetsPlatform } from '../renderer/platform'
...
export async function handleSave(
  ...
  if (mode === 'save' && state.file.csvPath !== undefined) {
    ...
    if (!confirmedCsvSaves.has(csvPath)) {
      const choice = await sheetsPlatform().api.confirmCsvSave()
      if (choice === 'cancel') {
  ...
  try {
    staged = await stageEditsForSave(sheetsPlatform().api, state.file.sessionId, edits)
  } catch (error: unknown) {
  ...
  try {
    ...
    const result = await sheetsPlatform().api.saveWorkbookEdits({
      sessionId: state.file.sessionId,
      mode,
    ...
    stashUndoCarry(null)
    try {
      const second = await sheetsPlatform().api.saveWorkbookEdits({
