...
function isRecord(value: unknown): value is LibraryRecord {
  ...
  return typeof record.path === 'string' && typeof record.openedAt === 'number' && typeof record.starred === 'boolean'
}
...
export function remember(records: readonly LibraryRecord[], path: string, now: number): LibraryRecord[] {
...
export function toggleStar(records: readonly LibraryRecord[], path: string, now: number): LibraryRecord[] {
  const found = records.find((record) => record.path === path)
  if (!found) return [{ path, openedAt: now, starred: true }, ...records]
  return records.map((record) => (record.path === path ? { ...record, starred: !record.starred } : record))
}

export function dropRecords(records: readonly LibraryRecord[], paths: readonly string[]): LibraryRecord[] {
...
/** `未命名.docx`, then `未命名 2.docx`, and so on. */
